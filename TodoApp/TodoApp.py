...
def homePage():
  ...
  return render_template('TodoApp.html', allTodo = allTodo)
...
def update(sn):
  if(request.method == 'POST'):
    titleUpdate = request.form['title']
    descriptionUpdate = request.form['description']
    todo = Todo.query.filter_by(sn=sn).first()
    todo.title = titleUpdate
    todo.description = descriptionUpdate
    db.session.add(todo)
    db.session.commit()
    return redirect('/')
  updateTodo = Todo.query.filter_by(sn=sn).first()  
  return render_template('update.html', updateTodo = updateTodo)
  # return f"{"this is update page for "} {updateTodo}"
